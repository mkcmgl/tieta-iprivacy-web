		...
		<el-card shadow="never" class="mt20 mb20 title-search">
			...
			<div>
				<el-table :data="list.tableData" v-loading="list.loading" style="width: 100%">
					...
					<el-table-column prop="strategyNames" label="加密策略" show-overflow-tooltip>
						<template #default="scope">
					...
					<el-table-column label="操作" align="center" width="160">
						<template #default="scope">
							...
							<el-button size="small" text type="info" @click="handleFiring(scope.row)"> 启动 </el-button>
...
import { getEncryptionTaskList, encryptionTaskStart, encryptionTaskDelete, dataTacticsDownload } from '@/api/dataEncryption';

const router = useRouter();

const AddTask = defineAsyncComponent(() => import('@/views/dataEncryption/addTask.vue'));
const TaskDetail = defineAsyncComponent(() => import('@/views/dataEncryption/taskDetail.vue'));
...
const deleteTask = (id: string) => {
	ElMessageBox.confirm('确定要删除该任务吗? 删除后不可恢复', '提示', {
		...
		encryptionTaskDelete({ id: id }).then((res) => {
			if (res.resultCode === 0) {
