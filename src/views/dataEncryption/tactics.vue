<template>
	<div class="system-user-container layout-padding">
		...
		<el-card shadow="never" class="mt20 mb20 title-search">
			...
			<div>
				<el-table :data="list.tableData" v-loading="list.loading" style="width: 100%">
					...
					<el-table-column prop="encryptionType" label="加密类型" show-overflow-tooltip>
						<template #default="scope">
							{{ scope.row.encryptionType == '0' ? '对称加密' : '非对称加密' }}
						</template>
					</el-table-column>
					<el-table-column prop="encryptionAlgorithm" label="加密算法" show-overflow-tooltip> </el-table-column>
		...
		<!-- <TacticsDetail ref="tacticsDetailRef" /> -->
	</div>
</template>
...
import { dataTacticsList, dataTacticsEdit, dataTacticsDelete } from '@/api/dataEncryption';

const AddTactics = defineAsyncComponent(() => import('@/views/dataEncryption/addTactics.vue'));
// const TacticsDetail = defineAsyncComponent(() => import('@/views/dataDesensitization/tacticsDetail.vue'));
...
const taskDetail = (id: string) => {
	// tacticsDetailRef.value?.openDialog();
};
...
const handleFiring = (row: any) => {
	ElMessageBox.confirm(`确定要${row.status === '0' ? '启动' : '停用'}该策略吗?`, '提示', {
