...
export default function () {
	...
	// 开始日期
	const compareStartDate = (startTime: string, endTime: string) => {
		let formatDate1 = new Date(startTime)
		let formatDate2 = new Date(endTime)
	...
	// 结束日期
	const compareEndDate = (startTime: string, endTime: string) => {
		let formatDate1 = new Date(startTime)
		let formatDate2 = new Date(endTime)
		if(formatDate1 <= formatDate2){
			return false
